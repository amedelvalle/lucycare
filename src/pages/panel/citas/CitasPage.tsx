...
// - Se agrega toggle Lista/Calendario en el header
// - Se extrae la vista de Lista a un render condicional
// - Se agrega render del CalendarView cuando toggle está en 'calendar'
// - NO se modifica la lógica existente: doctor_id, fecha, queries, handlers
...
export default function CitasPage() {
  ...
  const [view, setView] = useState<CitasView>('list');
  ...
  // ─── Stats rápidas (solo vista Lista) ─────────────────────────
  ...
  // ─── Render ────────────────────────────────────────────────────
  return (
    <div className={`p-6 mx-auto ${view === 'list' ? 'max-w-3xl' : 'max-w-6xl'}`}>
      {/* Header con toggle */}
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {view === 'list'
              ...
              : 'Vista de calendario. Cambia entre día, semana y mes.'}
          </p>
        </div>
        <ViewToggle view={view} onChange={setView} />
      </div>

      {/* Render condicional según toggle */}
      {view === 'list' ? (
