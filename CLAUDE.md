...
- **Fase 5** ✅ live (PR #98 diseño + PR #99/`s7_35` backend + PR #100 UI): **dedup preventivo** en creación de paciente/walk-in. RPC `find_patient_match_candidates` (`SECURITY DEFINER`): **intra-clínica → detalle** (id/nombre/teléfono/activo, "Usar este paciente"); **cross-clínica/global → solo señal mínima** (`weak` por teléfono, `strong` por documento), **sin PII**; gate `is_clinic_member`/`is_admin` (`P0001`); **advisory, no bloquea** (el bloqueo fuerte sigue siendo el `UNIQUE` intra-clínica). `createBasicPatient` con chequeo proactivo de documento → `DuplicateDocumentError` (banner amable, no error crudo). UI: `PatientMatchHints` reusable (debounce 350ms) en `NewPatientModal` + `CreateWalkInModal`. **Normalización del teléfono con `normalizePhoneSV`** (mismo helper que la dedup del INSERT) — el lookup y el submit ven lo mismo. Smoke s7_35 8/8 + OK visual. **Pendiente no-bloqueante:** RPC tolerante a teléfono crudo (defensa en profundidad, futura migración).
...
- ~~**Catálogos personalizados por médico** para diagnósticos y medicamentos~~ → ✅ **COMPLETO** (PRs #105–#111 + migración de datos, `s7_37`/`s7_38`/`s7_39`). Diseño en `docs/ANALISIS_CATALOGOS_PERSONALIZADOS.md`. Modelo global (base Lucy) + personal por médico; snapshot histórico; ocultar/clonar globales; base replicada migrada (Base Lucy 127/736, "Míos" solo lo propio). **Próximo frente recomendado (análisis hecho): UI de LucyAdmin para administrar la Base Lucy global.** **Backend YA listo** (no requiere migración): RLS `s7_38` (`*_insert_admin`/`*_update_admin` con `is_admin()`) ya permite a LucyAdmin CRUD de globales; `audit_catalog` ya audita; `s7_39` UNIQUE parcial ya evita duplicados; `s7_37` snapshot protege históricos (editar/inactivar un global no cambia recetas/consultas pasadas). **Diseño en `docs/ANALISIS_CATALOGOS_PERSONALIZADOS.md §7.** Plan: 1 PR **UI-only** (sin migración) — página **`/admin/catalogos`** + NavLink "Catálogos" en `AdminLayout`, gateada por `AdminOnlyRoute`; tabs **Diagnósticos + Medicamentos** Base Lucy (Antecedentes fuera); buscar · activos/inactivos · crear · editar · inactivar/reactivar · dedup (mensaje amable ante `23505`); `usage_count` oculto; **no hard-delete** (solo inactivar). Médicos/asistentes no editan globales (RLS). Preview + validación visual del owner antes del merge.
