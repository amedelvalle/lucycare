...
- ~~**Catálogos personalizados por médico** para diagnósticos y medicamentos~~ → ✅ **COMPLETO** (PRs #105–#111 + migración de datos, `s7_37`/`s7_38`/`s7_39`). Diseño en `docs/ANALISIS_CATALOGOS_PERSONALIZADOS.md`. Modelo global (base Lucy) + personal por médico; snapshot histórico; ocultar/clonar globales; base replicada migrada (Base Lucy 127/736, "Míos" solo lo propio). **Pendiente futuro (frente aparte):** UI de **LucyAdmin para administrar la Base Lucy global** — crear/editar/inactivar diagnósticos y medicamentos globales, audit, anti-duplicados; los médicos no editan la base (solo usan/ocultan/clonan), el impacto histórico ya está protegido por el snapshot de `s7_37`.
