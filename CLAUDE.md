...
## Estado actual del proyecto (snapshot 2026-06-05)

- **Fuente de verdad:** `origin/main` en GitHub (github.com/amedelvalle/lucycare).
- **HEAD esperado:** `1bf62c7` o posterior (+ el PR documental de cierre de esta ventana). **PRs #1–#96 mergeados** (#96 = cierre/handoff de la ventana del cambio de teléfono). Eje Correcciones post-firma COMPLETO (#74–#85, migraciones `s7_28..s7_31`). **Paciente Global Fases 1–3 COMPLETAS:** #87 (`s7_32`) identidad global + #88 UI `/paciente/perfil` + #90 (`s7_33`) sync espejo + guard + #91 UI médico read-only. **Cambio de teléfono por OTP COMPLETO:** #93 (`s7_34`) trigger de sync `auth.users.phone → profiles/patients` + audit + #94 UI paciente + #95 UI médico (`ChangePhoneModal`).
