...
- `docs/ANALISIS_ANALYTICS_LUCYCARE.md` — medición de tráfico/conversión de `lucycare.app` (PR-0 diseño, docs-only): Q1–Q5 cerradas (Vercel Web Analytics + Speed Insights **cookieless** como primario, GA4 descartado; `doctor_slug` como id de evento; **jamás PII/clínico ni texto libre de búsqueda**; dashboard DB LucyAdmin separado y posterior). Fases 1A (deps `@vercel/*`, solo rutas públicas) → 1B (Search Console, operativo owner) → 2 (eventos custom si el plan de Vercel los soporta) → 3 (dashboard conversión DB). **NADA implementado aún.**
...
- `docs/HANDOFF_CHATGPT_LUCYCARE_NUEVA_VENTANA_2026-06-21.md` — **PUNTO DE ENTRADA VIGENTE** (estado final post-#188, F4 cerrado, F4-D diferido, backlog vivo, protocolo de arranque). Leer este PRIMERO. Los handoffs `2026-06-18`, `2026-06-15` y `2026-06-11` quedan como históricos.
