...
- **⚠ OPERATIVO pendiente — nombre de Katherine NO persistió:** la corrección desde la ficha admin (caso "(sin nombre)" de `s7_42`) no llegó a la DB (`profiles.full_name` sigue vacío, sin audit de update, `updated_at` pre-fix). Rehacer desde `/admin/medicos/4bb793d0-44b1-4782-8896-6bf01cd627aa` > Perfil > Nombre completo y verificar que guarda (posible save fallido silencioso — si se repite, investigar como bug de la ficha).
