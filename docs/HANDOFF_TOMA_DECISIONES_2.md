...
2. **Confirmar HEAD final:** `c23dda1` o posterior. **Leer primero `docs/HANDOFF_LUCYCARE_NUEVA_VENTANA_2026-06-15.md`.**
3. **Confirmar PRs mergeados hasta #153.**
4. **Confirmar migraciones hasta `s7_49`** (aplicadas en Supabase). **#118 = `s7_40`**; **#120 = `s7_41`**; **#122 = `s7_42`** (re-aplicada completa tras 2 ajustes); **#128 = `s7_43`**; **#132 = `s7_44`**; **#135 = `s7_45`** (F4-1 claim tolerante); **#138 = `s7_46`** (F4-2 backend merge admin); **#140 = `s7_47`** (F4-3-search RPC candidatos); **#142 = UI PR A read-only (sin migración)**; **#144 = UI PR B merge real (sin migración)**; **#147 = `s7_48`** (Unmerge formal backend; re-aplicado 1 vez por fix de cast `document_type`); **#151 = `s7_49`** (F4-3b backend bandeja de rechazos; re-aplicado 1 vez por rollback del editor) + **#152** fix smoke + **#153** UI bandeja (sin migración).
...
- **Paciente Global Fase 4** — merge admin de duplicados. **Diseño cerrado (#134, DM1–DM9)** en `docs/ANALISIS_PACIENTE_GLOBAL_FASE4_MERGE_ADMIN.md`; **F4-1 (claim tolerante) ✅ live (#135/`s7_45`)**; **F4-2 (backend merge) ✅ live (#138/`s7_46`)** — `patient_merge_log` + `admin_merge_patients_preflight` (dry-run) + `admin_merge_patients` (transaccional) + bypass `app.merging_patients`; **V1 = `same_profile`**; neutraliza fuente sin hard-delete; bloqueos P0060–P0068. **F4-3-search (RPC de candidatos) ✅ live (#140/`s7_47`)** — `admin_list_patient_merge_candidates()` read-only/admin-only lista grupos `same_profile` (sin teléfono/nombre, sin contenido clínico, sin merge) + warning `W_UNCONFIRMED_LINK`. **F4-3 UI PR A read-only ✅ live (#142)** + **F4-3 UI PR B merge real ✅ live (#144)** — `/admin/pacientes` ya ejecuta la fusión real: botón "Fusionar fichas" solo si `preflight.eligible` + motivo ≥10 + confirmación reforzada (teclear `FUSIONAR FICHAS`) + `admin_merge_patients` + errores P0060–P0068 por `error.code` + invalidación de candidatos/historial; **sin unmerge, sin contenido clínico/JSON crudo, sin teléfono/nombre como evidencia, frontend-only**; validado con fixtures (apply→merge→P0061→clean, 0 residuales) + desktop/móvil + OK visual del owner. **Unmerge formal — backend ✅ live (#147/`s7_48`):** `admin_unmerge_patients_preflight` + `admin_unmerge_patients` revierten un merge sobre `patient_merge_log` (devuelve `moved_ids` destino→fuente solo las que siguen en el target, restaura la fuente desde `source_snapshot`, el destino conserva su historia nueva, rellena `unmerged_*` append-only + audit `admin_unmerge`, bypass propio `app.unmerging_patients`); reversa atómica, no hard-delete, no toca identidad global; códigos P0070–P0077; check 5/5 + smoke 17/17. Diseño en `docs/ANALISIS_PACIENTE_GLOBAL_F4_UNMERGE.md`. **Unmerge — UI "Deshacer fusión" ✅ live (#149):** acción en el historial de `/admin/pacientes` (botón solo si `unmerged_at IS NULL`; autoridad real = preflight; badge "Revertida" si ya revertida) → modal de 3 pasos (preflight con warnings como info → confirmación reforzada motivo ≥10 + teclear `DESHACER FUSIÓN` con tilde → resultado); errores P0070–P0077 por `error.code`; invalida historial + candidatos; sin contenido clínico/JSON crudo; frontend-only, `database.types.ts` no tocado. **Con esto, el eje F4 merge/unmerge queda cerrado de punta a punta (backend+UI de ambos).** **F4-3b V1 — bandeja de vínculos rechazados ✅ live (#151/`s7_49` backend + #152 fix smoke + #153 UI):** sección "Vínculos rechazados" en `/admin/pacientes` sobre la cola `patient_link_rejections` (rechazos "No son mías" de B2). Backend: columna `resolution_note` + `admin_list_patient_link_rejections(p_status)` (read admin-only, metadatos + estado actual de la ficha, sin contenido clínico) + `admin_resolve_link_rejection(p_rejection_id, p_resolution_note, p_reopen)` (resolver motivo ≥10 / reabrir; audit; reopen conserva estado previo; `P0080`/`P0081`); check 5/5 + smoke 11/11. UI (frontend-only): filtro pendientes/resueltos/todos + tabla + Resolver (modal, motivo ≥10) + Reabrir; invalida la query. **Resolver = bookkeeping** (no re-vincula, no fusiona, no toca `patients`/`profiles`/`auth.users`); la fila sigue excluyendo el par del auto-link mientras exista. **V1 = solo bandeja; SIN fuzzy, SIN detección proactiva, SIN F4-D.** Fixtures `F49_FIXTURE` (0 residuales); fila real `50375000001` restaurada a `pending_review` tras el preview. **Pendiente F4 (no arrancado):** **F4-D** (merge de identidades) diferida. (Frente delicado: mueve historial clínico, solo LucyAdmin, intra-clínica.)
