...
2. **Confirmar HEAD final:** `c559fd7` o posterior (+ el PR documental de refresh 2026-06-13). **Leer primero `docs/HANDOFF_LUCYCARE_NUEVA_VENTANA_2026-06-11.md`.**
3. **Confirmar PRs mergeados hasta #138** (+ el PR documental de refresh).
4. **Confirmar migraciones hasta `s7_46`** (aplicadas en Supabase). **#118 = `s7_40`**; **#120 = `s7_41`**; **#122 = `s7_42`** (re-aplicada completa tras 2 ajustes); **#128 = `s7_43`**; **#132 = `s7_44`**; **#135 = `s7_45`** (F4-1 claim tolerante); **#138 = `s7_46`** (F4-2 backend merge admin: `patient_merge_log` + preflight/merge RPCs + bypass `app.merging_patients`; V1 = `same_profile`).
