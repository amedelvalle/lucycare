...
`main` HEAD esperado: `5e27d38` o posterior (+ el PR documental de cierre de esta ventana), **PRs #1–#120 mergeados**. Migraciones hasta `s7_41` (aplicadas en Supabase; **#114 y #116 no traen migración**: #114 UI-only sobre el backend `s7_37`/`s7_38`/`s7_39` ya existente, #116 robustez frontend del panel; **#118 = `s7_40`**, **#120 = `s7_41`**). **Robustez PanelLayout/useClinicContext (#116):** `getSession()` (no `getUser()`) + timeout/`.catch()`/estado recuperable + errores tipados. **RPC F5 tolerante a teléfono crudo (#118/`s7_40`):** helper `normalize_phone_sv` + canonicalización en ambos lados; sin frontend. **Lista de espera global cross-médicos (#120/`s7_41`):** `/admin/lista-espera` + RPC `admin_list_waitlist` (join + filtros + paginación + `total_count`). Correcciones: #74–#85 (`s7_28..s7_31`). Paciente Global F2 #87 (`s7_32`) + #88, F3 #90 (`s7_33`) + #91. **Cambio de teléfono por OTP: #93 (`s7_34`) trigger de sync + #94 UI paciente + #95 UI médico (`ChangePhoneModal`).** **Paciente Global F5 (dedup preventivo): #98 diseño + #99 (`s7_35`) RPC `find_patient_match_candidates` + #100 UI `PatientMatchHints`.** **Mi equipo F2 (ciclo de vida de invitación): #102 (`s7_36`) backend + #103 UI `EquipoPage`.** **Catálogos personalizados COMPLETO: #105 diseño + #106 (`s7_37`) snapshot + #107 (`s7_38`) global+personal/RLS + #108 UI consulta + #109 UI admin + #110/#111 (`s7_39`) infra + migración de datos aplicada (Base Lucy 127/736, "Míos" solo lo propio, 11218 filas en `catalog_migration_map`).**
...
- **Afiliación médica — soportar creación de médico `listed_only` cuando el teléfono ya pertenece a un paciente existente.** Detectado al crear un médico desde Afiliaciones: la RPC `admin_approve_and_create_doctor` intenta crear un `auth.users` nuevo aunque el teléfono ya exista → falla con `users_phone_key` (error crudo visible para LucyAdmin). El problema de fondo NO es solo el copy: el flujo no contempla el caso válido "paciente existente → ahora también médico". **Frente separado (no parte de #120), en cola para análisis sin código.** Pasos a revisar: flujo Aprobar→Crear; dónde se crean `auth.users`/`profiles`/`clinics`/`doctors`; preflight por teléfono/email; clasificación de casos (paciente existente sin doctor → vincular y crear doctor/clinic sin nuevo auth.user; médico existente → bloquear con link; assistant/admin → revisión manual; inconsistencias → revisión manual); server-side atómico; copy amigable (nunca `duplicate key…`); el médico creado debe quedar `listed_only`/no publicado/no operativo/no verified; reclamo posterior sigue exigiendo OTP + licencia/JVPM + TOS; auditar el vínculo/reúso de profile. Ojo aprendizaje de §5.1: NO hacer OTP con el teléfono del médico antes de crear el doctor.
...
2. **Confirmar HEAD final:** `5e27d38` o posterior (+ el PR documental de cierre de esta ventana).
3. **Confirmar PRs mergeados hasta #116** (+ el PR documental de cierre de esta ventana).
4. **Confirmar migraciones hasta `s7_41`** (aplicadas en Supabase). **#114 y #116 no traen migración** (UI/frontend-only); **#118 = `s7_40`** (backend-only); **#120 = `s7_41`** (backend + UI admin).
...
- **Pendientes acotados:** paginación del Home. (~~causa raíz PanelLayout/useClinicContext~~ ✅ cerrada en #116; ~~RPC F5 tolerante a teléfono crudo~~ ✅ cerrada en #118/`s7_40`; ~~vista global `/admin/lista-espera`~~ ✅ cerrada en #120/`s7_41`.) **Nuevo frente detectado:** Afiliación médica — soportar creación de médico `listed_only` cuando el teléfono ya pertenece a un paciente existente (hoy falla con `users_phone_key`; análisis pendiente, ver §6.2).
