...
> (`docs/ANALISIS_*.md`).
>
> **Snapshot 2026-06-06** (ejes recién cerrados: **(1) Correcciones post-firma COMPLETO** — 5 bloques clínicos, PRs #74–#85, `s7_28..s7_31`; **(2) Paciente Global Fases 1–3 COMPLETAS** — identidad global (#87/`s7_32`) + UI `/paciente/perfil` (#88) + sync espejo + guard (#90/`s7_33`) + UI médico read-only (#91); **(3) Cambio de teléfono por OTP COMPLETO** — trigger de sync (#93/`s7_34`) + UI paciente (#94) + UI médico (#95); **(4) Paciente Global Fase 5 — dedup preventivo COMPLETO** (#98/#99/`s7_35`/#100); **(5) Mi equipo Fase 2 — ciclo de vida de invitación COMPLETO** (#102/`s7_36` + #103). Previo: Afiliación Fase 2 + smoke ✅; análisis pagos SaaS PR #62; ubicación estructurada admin PR #64/`s7_25` ✅ live).
>
...
`main` HEAD esperado: `3fa1604` o posterior (+ el PR documental de cierre de esta ventana), **PRs #1–#103 mergeados**. Migraciones hasta `s7_36` (aplicadas en Supabase). Correcciones: #74–#85 (`s7_28..s7_31`). Paciente Global F2 #87 (`s7_32`) + #88, F3 #90 (`s7_33`) + #91. **Cambio de teléfono por OTP: #93 (`s7_34`) trigger de sync + #94 UI paciente + #95 UI médico (`ChangePhoneModal`).** **Paciente Global F5 (dedup preventivo): #98 diseño + #99 (`s7_35`) RPC `find_patient_match_candidates` + #100 UI `PatientMatchHints`.** **Mi equipo F2 (ciclo de vida de invitación): #102 (`s7_36`) backend + #103 UI `EquipoPage`.**
...
2. **Confirmar HEAD final:** `3fa1604` o posterior (+ el PR documental de cierre de esta ventana).
3. **Confirmar PRs mergeados hasta #103** (+ el PR documental de cierre de esta ventana).
4. **Confirmar migraciones hasta `s7_36`** (aplicadas en Supabase).
...
- ~~**Mi equipo Fase 2** — expiración/reenvío de invitaciones.~~ ✅ hecho (#102/`s7_36` + #103).
...
- **Pendientes acotados:** vista global `/admin/lista-espera`, causa raíz PanelLayout/useClinicContext, catálogos personalizados por médico (diagnósticos/medicamentos), paginación del Home.
