...
2. **Confirmar HEAD final:** `19a597f` o posterior (+ el PR documental de handoff 2026-06-15). **Leer primero `docs/HANDOFF_LUCYCARE_NUEVA_VENTANA_2026-06-15.md`.**
3. **Confirmar PRs mergeados hasta #142** (+ el PR documental de handoff).
4. **Confirmar migraciones hasta `s7_47`** (aplicadas en Supabase). **#118 = `s7_40`**; **#120 = `s7_41`**; **#122 = `s7_42`** (re-aplicada completa tras 2 ajustes); **#128 = `s7_43`**; **#132 = `s7_44`**; **#135 = `s7_45`** (F4-1 claim tolerante); **#138 = `s7_46`** (F4-2 backend merge admin); **#140 = `s7_47`** (F4-3-search RPC candidatos); **#142 = UI PR A read-only (sin migración)**.
...
- **Paciente Global Fase 4** — merge admin de duplicados. **Diseño cerrado (#134, DM1–DM9)** en `docs/ANALISIS_PACIENTE_GLOBAL_FASE4_MERGE_ADMIN.md`; **F4-1 (claim tolerante) ✅ live (#135/`s7_45`)**; **F4-2 (backend merge) ✅ live (#138/`s7_46`)** — `patient_merge_log` + `admin_merge_patients_preflight` (dry-run) + `admin_merge_patients` (transaccional) + bypass `app.merging_patients`; **V1 = `same_profile`**; neutraliza fuente sin hard-delete; bloqueos P0060–P0068. **F4-3-search (RPC de candidatos) ✅ live (#140/`s7_47`)** — `admin_list_patient_merge_candidates()` read-only/admin-only lista grupos `same_profile` (sin teléfono/nombre, sin contenido clínico, sin merge) + warning `W_UNCONFIRMED_LINK`. **F4-3 UI PR A read-only ✅ live (#142)** — `/admin/pacientes`: lista de grupos + comparación source/target + preflight + historial; **sin botón Fusionar, sin `admin_merge_patients`, sin contenido clínico/JSON crudo**. **Próximo: F4-3 UI PR B** = habilitar el merge real (botón "Fusionar" solo si elegible + motivo obligatorio + confirmación explícita + `admin_merge_patients` + refresh; validación solo con fixtures, nunca pacientes reales). Luego **F4-3b** bandeja `patient_link_rejections`, **unmerge formal** (`admin_unmerge_patients`; el log ya nace con `source_snapshot`/`moved_ids`/`unmerge_*`) y **F4-D** (identidades) diferida. **Decisión A/B:** F4-3 UI se dividió en read-only (A, #142) + destructiva (B, pendiente) por seguridad — primera UI admin que ejecuta merge, sin unmerge aún. (Frente delicado: mueve historial clínico, solo LucyAdmin, intra-clínica.)
