-- ═══════════════════════════════════════════════════════════
-- Migración S7-31: B1.5-a — inmutabilidad de vitals + amend de
-- diagnósticos / antecedentes / signos vitales
-- ═══════════════════════════════════════════════════════════
-- Cierra el eje Correcciones post-firma para los 3 bloques clínicos que
-- faltaban. Dos partes:
--
-- (1) INMUTABILIDAD DE VITALS (hueco que s7_28 dejó explícito):
--     s7_28 endureció consultations/prescriptions/diagnoses/family_history
--     con signed_at IS NULL, pero NO vitals (se gatea por appointment_id, sin
--     signed_at propio — s7_26 lo dejó solo doctor-scoped). → Hoy un médico
--     puede editar por API directa los signos vitales de una cita cuya consulta
--     ya está firmada, en silencio. Lo cerramos: vitals_update / vitals_insert
--     rechazan si la cita tiene una consulta FIRMADA. La corrección controlada
--     pasa por amend_consultation (SECURITY DEFINER, BYPASSRLS) → único camino.
--
-- (2) EXTENDER amend_consultation con p_diagnosis_ops / p_family_history_ops /
--     p_vitals_changes. Se corrigen IN-PLACE (sin versionado, a diferencia de
--     recetas que se imprimen): la traza vive en consultation_amendments
--     (snapshot before/after — consultation_snapshot ya captura diagnoses,
--     family_history y vitals) + audit. affects_prescriptions NO cambia: sigue
--     dependiendo solo de p_prescription_ops → corregir solo estos bloques deja
--     affects_prescriptions=false (no marca "RECETA CORREGIDA").
--
-- Se mantiene: motivo obligatorio, snapshot before/after, audit, rechazo de
-- campos prohibidos (P0013), no tocar paciente/médico/fecha/firma/signed_at,
-- solo médico dueño, asistente bloqueado.
-- ═══════════════════════════════════════════════════════════

-- ─── 1. Inmutabilidad de vitals (cita con consulta firmada) ─────────
DROP POLICY IF EXISTS vitals_update ON vitals;
CREATE POLICY vitals_update ON vitals
  FOR UPDATE USING (
    appointment_id IN (SELECT a.id FROM appointments a WHERE a.doctor_id = get_user_doctor_id())
    AND NOT EXISTS (
      SELECT 1 FROM consultations c
       WHERE c.appointment_id = vitals.appointment_id AND c.signed_at IS NOT NULL
    )
  );
...
-- vitals_select (s7_26) y la ausencia de vitals_delete se conservan.

-- ─── 2. amend_consultation extendida ────────────────────────────────
-- Se agregan params → nueva firma. DROP de la firma vieja (5 args) para no
-- dejar dos overloads. Los llamados existentes (B2) pasan 4 args nombrados →
-- resuelven con los defaults de la firma nueva.
DROP FUNCTION IF EXISTS amend_consultation(uuid, text, jsonb, jsonb, text);
  ...
  FOR v_op IN SELECT jsonb_array_elements(COALESCE(p_prescription_ops, '[]'::jsonb)) LOOP
    ...
    IF v_action = 'replace' THEN
      ...
      INSERT INTO prescriptions (
        ...
        v_old.alternatives, v_old.version + 1, true, v_old.id
      );
