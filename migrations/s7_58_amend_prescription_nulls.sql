-- ═══════════════════════════════════════════════════════════
-- Migración S7-58: F6 — corrección post-firma de receta que
--                  realmente puede LIMPIAR campos (NULL)
-- ═══════════════════════════════════════════════════════════
-- PROBLEMA (confirmado en auditoría clínica):
--   El bloque de receta de `amend_consultation` (s7_31) usaba
--     COALESCE(NULLIF(v_op->>'duration_value',''), v_old.duration_value)
--   es decir, decidía por el VALOR y no por la PRESENCIA de la clave. Como el
--   cliente omitía `duration_value` cuando el médico lo dejaba vacío (o al pasar
--   a 'permanente'), el COALESCE restauraba el valor viejo.
--
--   Consecuencia: el médico borraba la duración, el sistema creaba la adenda,
--   versionaba la receta (v2), marcaba "RECETA CORREGIDA" y la imprimía con
--   "Corrección v2"… pero la duración vieja SOBREVIVÍA. El sistema afirmaba
--   haber corregido algo que no corrigió, con sello de corrección.
--
--   Además `dosage`/`frequency`/`instructions` se guardaban como '' (cadena
--   vacía) al borrarlos, en vez de NULL como en el borrador y en el `add`.
--
-- FIX (quirúrgico, SOLO el bloque de receta):
--   Se adopta el MISMO patrón que la propia RPC ya usa para vitales, notas de
--   diagnóstico y antecedentes — presencia de clave, no COALESCE ciego:
--
--     clave ausente              → conservar el valor anterior
--     clave presente (null / '') → limpiar (NULL)
--     clave presente con valor   → actualizar
--
--   Y una regla clínica server-side: si `duration_unit = 'permanente'`, el
--   `duration_value` queda SIEMPRE en NULL (no hay duración numérica fantasma),
--   sin depender de que el cliente la mande bien.
--
-- NO CAMBIA (idéntico a s7_31): firma de la función, gates (médico dueño,
...
--   consultation_amendments, affects_prescriptions, audit_log, y los bloques de
--   diagnósticos, antecedentes y vitales.
--
-- FUERA DE ALCANCE: `alternatives` sigue conservándose tal cual en el `replace`
--   y sin poder asignarse en el `add` (eso es F7, frente aparte). Sin backfill:
--   las filas históricas con '' o con duración fantasma quedan como están.
-- ═══════════════════════════════════════════════════════════
  ...
  FOR v_op IN SELECT jsonb_array_elements(COALESCE(p_prescription_ops, '[]'::jsonb)) LOOP
    ...
    IF v_action = 'replace' THEN
      ...
      INSERT INTO prescriptions (
        ...
        -- F7 (fuera de alcance): las alternativas se conservan tal cual.
